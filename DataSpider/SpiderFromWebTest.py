...
def getDataFrom(url):
    ...
    li_label = r'"provinceName":(.*?)00'
    li_data =get_data(table[2],li_label)
    ...
    print('---------------------------span_label---------------------------')
    #指定城市名的标签
    span_city_label = r'provinceShortName"(.*?)c'
    #指定省名的标签
    span_pro_label = r'currentConfirmedCount"(.*?)c'
    #指定温度的标签
    span_wd_label = r'confirmedCount"(.*?)s'
    ...
    if len(li_data)>0:
        ...
        for i in range(ncount_li_data):
            ...
            if len(subtable_wd):
                ...
                print('总确诊：'+str(wd_value))
                if wd_value[0] is not None:
                    sub_list.append(wd_value[0])
            illness_list.append(sub_list)
            print(illness_list)
    #准备数据集
    pd_data = pd.DataFrame(illness_list)
    #表格列名，为输出EXCEL做准备
    colunms=['Num','Prov','currentConfirmedCount','ConfirmedCount']
    #添加列名
    pd_data.columns=colunms
    print(pd_data)
    ExcelName=r'D:\pycharmProject\PyWebSpider\Data\IllnessArray1.xlsx'
    sheetName='Illness'
